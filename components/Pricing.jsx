    ...
    <section className="section-spacing" aria-labelledby="pricing-heading">
      <div className="section-container">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {cards.map((card, i) => (
            <motion.article key={card.title} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: i * 0.05, ease: [0.22, 1, 0.36, 1] }} viewport={{ once: true, amount: 0.15 }} className={`card relative p-6 ${card.featured ? 'border-2 border-[var(--accent-purple)]' : ''}`}>
